	...
	switch ($sOperation) {
		case 'select_precanned':
			...
			if (PrecannedRepliesPlugIn::UseLegacy()) {
				...
				$sHTML .= $oBlock->GetDisplay($oPage, 'precanned_select', ['open' => true, 'currentId' => 'precanned_select', 'table_inner_id' => 'datatable_search_form_result_precanned_select']);
				...
				$oBlock->Display($oPage, 'precanned_select', ['open' => true, 'currentId' => 'precanned_select', 'table_inner_id' => 'datatable_search_form_result_precanned_select',				'menu' => false,
				 'selection_mode' => true,
		...
		case 'search_precanned':
			...
			$oBlock->Display($oPage, 'search_form_result_precanned_select', ['cssCount' => '#count_precanned_select', 'menu' => false, 'selection_mode' => true, 'selection_type' => 'single']); // Don't display the 'Actions' menu on the results
			break;
		...
		case 'add_precanned':
			...
			} elseif (!PrecannedRepliesPlugIn::UseLegacy() && $sObjClass != '' && $sObjId > 0) {
			...
			foreach ($aSelected as $iId) {
				...
				if ($oPR != null) {
					...
					$oSet = new DBObjectSet($oSearch, [], ['class' => get_class($oPR), 'item_id' => $oPR->GetKey()]);
					$aAtt = [];
